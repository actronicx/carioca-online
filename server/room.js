...
function pescaDalMazzo(stato, idx) {
  ...
  // Ha rifiutato la carta in cima al pozzo (se c'era): si apre una finestra di 10s in cui
  // chiunque altro può segnalare interesse; passato il tempo, vince chi è di turno più vicino.
  stato.scartoReclamabile = stato.pozzo.length > 0
    ? { cartaId: stato.pozzo[stato.pozzo.length - 1].id, giocatoreCheHaRifiutato: idx, richiedenti: [] }
    : null;
  return { ok: true };
}
