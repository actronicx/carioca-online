...
// codice stanza -> timeout handle della finestra di 10s per reclamare uno scarto rifiutato
const timerScartoReclamabile = new Map();

const DURATA_FINESTRA_SCARTO_MS = 10000;

function annullaTimerScarto(codice) {
  if (timerScartoReclamabile.has(codice)) {
    clearTimeout(timerScartoReclamabile.get(codice));
    timerScartoReclamabile.delete(codice);
  }
}

function avviaTimerScartoSeServe(codice, stato) {
  annullaTimerScarto(codice);
  if (!stato.scartoReclamabile) return;
  const timer = setTimeout(() => {
    timerScartoReclamabile.delete(codice);
    const statoAttuale = stanze.get(codice);
    if (!statoAttuale) return;
    RoomLogic.risolviScartoReclamabile(statoAttuale);
    inviaStatoATutti(codice);
  }, DURATA_FINESTRA_SCARTO_MS);
  timerScartoReclamabile.set(codice, timer);
}
...
io.on("connection", (socket) => {
  ...
  socket.on("azione", ({ codice, tipo, payload }, callback) => {
    ...
    switch (tipo) {
      case "sceltaMano":
        risultato = RoomLogic.sceltaMano(stato, idx, payload.idMano, payload.alBuio);
        break;
      case "pescaMazzo":
        risultato = RoomLogic.pescaDalMazzo(stato, idx);
        if (risultato.ok) avviaTimerScartoSeServe(codice, stato);
        break;
      case "pescaPozzo":
        risultato = RoomLogic.pescaDalPozzo(stato, idx);
        break;
      case "reclamaScarto":
        risultato = RoomLogic.registraRichiestaScarto(stato, idx);
        break;
      case "scarta":
        risultato = RoomLogic.scarta(stato, idx, payload.idCarta);
        if (risultato.ok) annullaTimerScarto(codice);
        break;
      case "confermaScendi":
      ...
      case "confermaScendiLibero":
        risultato = RoomLogic.confermaScendiLibero(stato, idx, payload.tipo, payload.ids, payload.dichiarazioniJolly);
        break;
      case "attaccaCarte":
        risultato = RoomLogic.attaccaCarte(stato, idx, payload.giocatoreTarget, payload.gruppoIdx, payload.ids, payload.dichiarazioniJolly);
        break;
      case "riscattaJolly":
        risultato = RoomLogic.riscattaJolly(stato, idx, payload);
        break;
    ...
    callback?.(risultato);
    if (risultato.ok) {
      if (stato.fase !== "gioco") annullaTimerScarto(codice);
      inviaStatoATutti(codice);
    }
  });
