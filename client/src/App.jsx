...
const SEMI_LISTA = ["cuori", "quadri", "fiori", "picche"];

// Piccolo selettore per dichiarare quale carta rappresenta un Jolly in un tris/poker
function SceltaSemeJolly({ cartaJolly, valoreGruppo, semeScelto, onScegli }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", background: "rgba(0,0,0,0.2)", borderRadius: 6, padding: "4px 8px" }}>
      <span style={{ fontSize: 11, color: "#F5E9D3AA" }}>Jolly = {valoreGruppo} di:</span>
      {SEMI_LISTA.map((s) => (
        <button
          key={s}
          onClick={() => onScegli(cartaJolly.id, s)}
          style={{
            padding: "3px 8px", borderRadius: 5, fontSize: 13, cursor: "pointer",
            border: semeScelto === s ? "2px solid #D4AF37" : "1px solid rgba(245,233,211,0.3)",
            background: semeScelto === s ? "#D4AF37" : "rgba(255,255,255,0.06)",
            color: semeScelto === s ? "#0B4D3A" : (s === "cuori" || s === "quadri") ? "#C1272D" : "#F5E9D3",
            fontWeight: 700,
          }}
        >
          {SEMI_SIMBOLO[s]}
...
export default function App() {
  ...
  const [dichiarazioniJollyPendenti, setDichiarazioniJollyPendenti] = useState({}); // { [idCartaJolly]: seme }
  const [attaccoInSospeso, setAttaccoInSospeso] = useState(null); // { giocatoreTarget, gruppoIdx } quando serve dichiarare un jolly prima di attaccare
  const [modalitaRiscattoJolly, setModalitaRiscattoJolly] = useState(null); // { giocatoreTarget, gruppoIdx, idJollyCarta, dichiarazione, idCartaPropria }
  const [riscattoTipo, setRiscattoTipo] = useState(null); // 'tris' | 'poker' | 'scala'
  const [riscattoNuoviIds, setRiscattoNuoviIds] = useState([]);
  const [riscattoDichiarazioni, setRiscattoDichiarazioni] = useState({});
  ...
  const [modalitaScala40, setModalitaScala40] = useState(false);
  const [gruppiScala40, setGruppiScala40] = useState([]);
  const [erroreAzione, setErroreAzione] = useState("");
  const [secondiRimastiScarto, setSecondiRimastiScarto] = useState(10);
  ...
  // Conto alla rovescia visivo (10s) per la finestra di reclamo dello scarto rifiutato
  useEffect(() => {
    const cartaId = stato?.scartoReclamabile?.cartaId || null;
    if (!cartaId) {
      setSecondiRimastiScarto(10);
      return;
    }
    setSecondiRimastiScarto(10);
    const intervallo = setInterval(() => {
      setSecondiRimastiScarto((s) => (s > 0 ? s - 1 : 0));
    }, 1000);
    return () => clearInterval(intervallo);
  }, [stato?.scartoReclamabile?.cartaId]);
  ...
  // Popup con lo storico completo dei risultati, consultabile in qualsiasi momento della partita
  function renderRisultatiModal() {
    if (!mostraRisultati || !stato || !stato.nomi) return null;
    return (
      <div
        onClick={() => setMostraRisultati(false)}
        style={{
          position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100,
          display: "flex", alignItems: "center", justifyContent: "center", padding: 20,
        }}
      >
        <div onClick={(e) => e.stopPropagation()} style={{ ...styles.setupCard, maxWidth: 560, maxHeight: "80vh", overflowY: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h2 style={{ ...styles.titolo, fontSize: 24, margin: 0 }}>Risultati</h2>
            <button onClick={() => setMostraRisultati(false)} style={{ ...styles.annullaBtn, padding: "6px 12px" }}>Chiudi</button>
          </div>
          {stato.storicoPunteggi.length === 0 ? (
            <p style={{ color: "#F5E9D3AA", textAlign: "center" }}>Nessuna mano ancora completata.</p>
          ) : (
            <table style={styles.tabella}>
              <thead>
                <tr>
                  <th style={styles.th}>Mano</th>
                  {stato.nomi.map((n, i) => <th key={i} style={styles.th}>{n}</th>)}
                </tr>
              </thead>
              <tbody>
                {stato.storicoPunteggi.map((r, ri) => (
                  <tr key={ri}>
                    <td style={styles.td}>{r.manoNome}</td>
                    {r.punti.map((p, i) => (
                      <td key={i} style={{ ...styles.td, fontWeight: i === r.vincitore ? 700 : 400 }}>
                        {p}{i === r.vincitore ? <span style={{ fontSize: 11 }}> 👑</span> : ""}
                      </td>
                    ))}
                  </tr>
                ))}
                <tr>
                  <td style={{ ...styles.td, fontWeight: 700, color: "#D4AF37", borderTop: "2px solid rgba(212,175,55,0.4)" }}>Totale</td>
                  {stato.punteggi.map((p, i) => (
                    <td key={i} style={{ ...styles.td, fontWeight: 700, color: "#D4AF37", borderTop: "2px solid rgba(212,175,55,0.4)" }}>{p}</td>
                  ))}
                </tr>
              </tbody>
            </table>
    ...
    return (
      <>
        <div style={styles.tavoloSetup}>
          <style>{fontImport}</style>
          <div style={styles.setupCard}>
            <h2 style={{ ...styles.titolo, fontSize: 30 }}>Fine mano {stato.round + 1}/10 — {ultimo.manoNome}</h2>
              ...
              <tbody>
                {stato.nomi.map((n, i) => (
                  <tr key={i}>
                    ...
                    <td style={{ ...styles.td, fontWeight: i === ultimo.vincitore ? 700 : 400 }}>
                      {ultimo.punti[i]}{i === ultimo.vincitore ? <span style={{ fontSize: 11 }}> 👑</span> : ""}
                    </td>
            ...
            <button onClick={() => setMostraRisultati(true)} style={{ ...styles.annullaBtn, marginTop: 16, width: "100%" }}>
  ...
  return (
    ...
    <div style={styles.tavolo}>
      ...
      <div style={styles.header}>
        <div>
          <div style={styles.manoLabel}>
            {stato.manoAttuale ? `Mano ${stato.round + 1}/10 — ${stato.manoAttuale.nome}` : `Round ${stato.round + 1}/10 — in attesa della scelta`}
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
          <button onClick={() => setMostraRisultati(true)} style={{ ...styles.annullaBtn, padding: "6px 12px", fontSize: 12 }}>
            Risultati
          </button>
        ...
        <div style={styles.scendiBox}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {stato.maniDisponibili
              ...
              .map((m) => (
                <div key={m.id} style={styles.opzioneManoBtn}>
                  <div style={{ fontWeight: 700, fontFamily: "'Fraunces', serif", fontSize: 16 }}>{m.nome}</div>
                  <div style={{ fontSize: 12, color: "#F5E9D3AA", marginTop: 2, marginBottom: 8 }}>{m.desc}</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button
                      onClick={() => { emitAzione("sceltaMano", { idMano: m.id, alBuio: false }); setModalitaSceltaMano(false); }}
                    ...
                    {m.id === 8 && (
                      <button
                        onClick={() => { emitAzione("sceltaMano", { idMano: m.id, alBuio: true }); setModalitaSceltaMano(false); }}
                        style={{ ...styles.confermaBtn, background: "#C1272D", color: "#F5E9D3" }}
                      >
                        Scegli al buio (-40 se vinci, invece di -20)
                      </button>
                    )}
                  </div>
                </div>
            ...
            <div key={gi} style={styles.righeGiocatore}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {giocCombo.map((grp, gpi) => {
                  const jollyDelGruppo = grp.carte.filter((c) => c.jolly && grp.jollyDichiarati?.[c.id]);
                  return (
                    <div
                      key={gpi}
                      onClick={() => {
                        if (!sonoIoDiTurno || modalitaScendi || modalitaScendiLibero || modalitaChiusura || modalitaScala40 || !stato.sceso[mioIndice] || selezione.length === 0) return;
                        const carteSelezionate = selezione.map((id) => manoGiocatore.find((c) => c.id === id)).filter(Boolean);
                        const eScala = grp.tipo.toLowerCase().includes("scala");
                        if (!eScala && carteSelezionate.some((c) => c.jolly)) {
                          setAttaccoInSospeso({ giocatoreTarget: gi, gruppoIdx: gpi });
                        } else {
                    ...
                    >
                      ...
                      {jollyDelGruppo.map((j) => {
                        const dich = grp.jollyDichiarati[j.id];
                        const possoRiscattare = sonoIoDiTurno && stato.sceso[mioIndice] &&
                          manoGiocatore.some((c) => !c.jolly && c.valore === dich.valore && c.seme === dich.seme);
                        return (
                          <div key={j.id} style={{ fontSize: 10, color: "#F5E9D3AA", marginTop: 4, display: "flex", alignItems: "center", gap: 6 }}>
                            <span>🃏 = {dich.valore} {SEMI_SIMBOLO[dich.seme]}</span>
                            {possoRiscattare && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  const cartaPropria = manoGiocatore.find((c) => !c.jolly && c.valore === dich.valore && c.seme === dich.seme);
                                  setModalitaRiscattoJolly({ giocatoreTarget: gi, gruppoIdx: gpi, idJollyCarta: j.id, dichiarazione: dich, idCartaPropria: cartaPropria.id });
                                  setRiscattoNuoviIds([]);
                                  setRiscattoTipo(null);
                                  setRiscattoDichiarazioni({});
                                }}
                                style={{ background: "#D4AF37", color: "#0B4D3A", border: "none", borderRadius: 4, padding: "2px 6px", fontSize: 10, fontWeight: 700, cursor: "pointer" }}
                              >
                                Riscatta
                              </button>
                            )}
      ...
      {/* Dichiarazione jolly richiesta prima di completare un attacco */}
      {attaccoInSospeso && (
        <div style={styles.scendiBox}>
          <div style={{ fontWeight: 700, color: "#D4AF37", marginBottom: 8 }}>
            Stai aggiungendo un Jolly a questa combinazione: specifica quale carta rappresenta
          </div>
          {(() => {
            const carteSelezionate = selezione.map((id) => manoGiocatore.find((c) => c.id === id)).filter(Boolean);
            const jollyDaDichiarare = carteSelezionate.filter((c) => c.jolly);
            const valoreGruppo = stato.combinazioniScese[attaccoInSospeso.giocatoreTarget][attaccoInSospeso.gruppoIdx].carte.find((c) => !c.jolly)?.valore
              || carteSelezionate.find((c) => !c.jolly)?.valore || "?";
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {jollyDaDichiarare.map((j) => (
                  <SceltaSemeJolly
                    key={j.id}
                    cartaJolly={j}
                    valoreGruppo={valoreGruppo}
                    semeScelto={dichiarazioniJollyPendenti[j.id]}
                    onScegli={(idJ, seme) => setDichiarazioniJollyPendenti((d) => ({ ...d, [idJ]: seme }))}
                  />
                ))}
              </div>
            );
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <button
              onClick={() => {
                emitAzione("attaccaCarte", { ...attaccoInSospeso, ids: selezione, dichiarazioniJolly: dichiarazioniJollyPendenti });
                setSelezione([]);
                setAttaccoInSospeso(null);
                setDichiarazioniJollyPendenti({});
              }}
              style={styles.confermaBtn}
            >
              Conferma attacco
            </button>
            <button onClick={() => { setAttaccoInSospeso(null); setDichiarazioniJollyPendenti({}); }} style={styles.annullaBtn}>Annulla</button>
          </div>
        </div>
      )}

      {/* Riscatto del jolly: scegli con quali carte (incluso il jolly appena ripreso) formare subito una nuova combinazione */}
      {modalitaRiscattoJolly && (
        <div style={styles.scendiBox}>
          <div style={{ fontWeight: 700, color: "#D4AF37", marginBottom: 8 }}>
            Riscatti il Jolly con {modalitaRiscattoJolly.dichiarazione.valore} di {modalitaRiscattoJolly.dichiarazione.seme} — ora devi usarlo SUBITO in una nuova combinazione:
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <button onClick={() => setRiscattoTipo("tris")} style={riscattoTipo === "tris" ? styles.confermaBtn : styles.annullaBtn}>Tris</button>
            <button onClick={() => setRiscattoTipo("poker")} style={riscattoTipo === "poker" ? styles.confermaBtn : styles.annullaBtn}>Poker</button>
            <button onClick={() => setRiscattoTipo("scala")} style={riscattoTipo === "scala" ? styles.confermaBtn : styles.annullaBtn}>Scala</button>
          </div>
          {riscattoTipo && (
            <>
              <div style={{ fontSize: 12, color: "#F5E9D3AA", marginBottom: 6 }}>
                Il Jolly riscattato è già incluso automaticamente. Tocca in mano le altre carte da aggiungere:
              </div>
              <div style={{ display: "flex", gap: 4, minHeight: 56, flexWrap: "wrap", marginBottom: 8 }}>
                <Carta carta={{ id: modalitaRiscattoJolly.idJollyCarta, jolly: true }} piccola />
                {riscattoNuoviIds.map((id) => {
                  const c = manoGiocatore.find((cc) => cc.id === id && id !== modalitaRiscattoJolly.idCartaPropria);
                  return c ? <Carta key={id} carta={c} piccola /> : null;
                })}
              </div>
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 10 }}>
                {manoGiocatore.filter((c) => c.id !== modalitaRiscattoJolly.idCartaPropria).map((c) => (
                  <Carta
                    key={c.id}
                    carta={c}
                    piccola
                    selezionata={riscattoNuoviIds.includes(c.id)}
                    onClick={() => setRiscattoNuoviIds((ids) => (ids.includes(c.id) ? ids.filter((x) => x !== c.id) : [...ids, c.id]))}
                  />
                ))}
              </div>
              {riscattoTipo !== "scala" && (() => {
                const carteScelte = [{ id: modalitaRiscattoJolly.idJollyCarta, jolly: true }, ...riscattoNuoviIds.map((id) => manoGiocatore.find((c) => c.id === id)).filter(Boolean)];
                const jollyNelNuovoGruppo = carteScelte.filter((c) => c.jolly);
                const valoreGruppo = carteScelte.find((c) => !c.jolly)?.valore || modalitaRiscattoJolly.dichiarazione.valore;
                return (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
                    {jollyNelNuovoGruppo.map((j) => (
                      <SceltaSemeJolly
                        key={j.id}
                        cartaJolly={j}
                        valoreGruppo={valoreGruppo}
                        semeScelto={riscattoDichiarazioni[j.id]}
                        onScegli={(idJ, seme) => setRiscattoDichiarazioni((d) => ({ ...d, [idJ]: seme }))}
                      />
                    ))}
                  </div>
                );
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => {
                emitAzione("riscattaJolly", {
                  ...modalitaRiscattoJolly,
                  nuovoGruppoIds: [modalitaRiscattoJolly.idJollyCarta, ...riscattoNuoviIds],
                  tipoNuovoGruppo: riscattoTipo,
                  dichiarazioniJolly: riscattoDichiarazioni,
                });
                setModalitaRiscattoJolly(null);
                setRiscattoNuoviIds([]);
                setRiscattoTipo(null);
                setRiscattoDichiarazioni({});
              }}
              disabled={!riscattoTipo}
              style={{ ...styles.confermaBtn, opacity: riscattoTipo ? 1 : 0.4 }}
            >
              Conferma riscatto
            </button>
            <button
              onClick={() => { setModalitaRiscattoJolly(null); setRiscattoNuoviIds([]); setRiscattoTipo(null); setRiscattoDichiarazioni({}); }}
              style={styles.annullaBtn}
            >
              Annulla
            </button>
          </div>
      ...
      {/* Scarto rifiutato reclamabile: finestra di 10s in cui chiunque altro può segnalare interesse.
          Passato il tempo, il server assegna la carta a chi è di turno più vicino a chi l'ha rifiutata. */}
      {stato.manoAttuale && stato.scartoReclamabile && stato.pozzo.length > 0 && (
        <div style={{ ...styles.messaggio, display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <span>
              🎴 {stato.nomi[stato.scartoReclamabile.giocatoreCheHaRifiutato]} ha rifiutato la carta in cima al pozzo — chi è interessato ha ancora <strong>{secondiRimastiScarto}s</strong>!
            </span>
            {mioIndice !== stato.turno && (
              stato.scartoReclamabile.richiedenti.includes(mioIndice) ? (
                <span style={{ color: "#D4AF37", fontWeight: 600, fontSize: 13 }}>✓ Interesse segnalato</span>
              ) : (
                <button onClick={() => emitAzione("reclamaScarto")} style={styles.azioneBtn}>
                  Sono interessato
                </button>
              )
            )}
          </div>
          {stato.scartoReclamabile.richiedenti.length > 0 && (
            <div style={{ fontSize: 12, color: "#F5E9D3AA" }}>
              Interessati finora: {stato.scartoReclamabile.richiedenti.map((i) => stato.nomi[i]).join(", ")}
            </div>
          )}
        </div>
      )}
      ...
      {modalitaScendiLibero && (
        <div style={styles.scendiBox}>
          ...
          {tipoScendiLibero !== "scala" && (() => {
            const carteSelezionate = selezione.map((id) => manoGiocatore.find((c) => c.id === id)).filter(Boolean);
            const jollyNelGruppo = carteSelezionate.filter((c) => c.jolly);
            const valoreGruppo = carteSelezionate.find((c) => !c.jolly)?.valore || "?";
            if (jollyNelGruppo.length === 0) return null;
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 10 }}>
                {jollyNelGruppo.map((j) => (
                  <SceltaSemeJolly
                    key={j.id}
                    cartaJolly={j}
                    valoreGruppo={valoreGruppo}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <button
              onClick={() => {
                emitAzione("confermaScendiLibero", { tipo: tipoScendiLibero, ids: selezione, dichiarazioniJolly: dichiarazioniJollyPendenti });
            ...
            <button onClick={() => { setModalitaScendiLibero(false); setTipoScendiLibero(null); setSelezione([]); setDichiarazioniJollyPendenti({}); }} style={styles.annullaBtn}>Annulla</button>
          </div>
        </div>
      )}
      ...
      {/* Scala 40: componi liberamente tris/poker/scale finché insieme valgono almeno 40 punti */}
      {modalitaScala40 && (
        <div style={styles.scendiBox}>
          <div style={{ fontWeight: 700, color: "#D4AF37", marginBottom: 8 }}>
            Componi tris, poker o scale (3+ carte, niente coppie o full) finché insieme valgono almeno 40 punti. Il jolly vale come la carta che sostituisce; l'Asso vale 1 in scala prima del 2, altrimenti 11.
          </div>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {gruppiScala40.map((gruppo, idx) => (
              <div
                key={idx}
                style={styles.gruppoSlot}
                onClick={() => {
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <span style={{ fontSize: 11, color: "#F5E9D3AA" }}>Gruppo {idx + 1}</span>
                  {gruppiScala40.length > 1 && (
                    <button
                      onClick={(e) => { e.stopPropagation(); setGruppiScala40((g) => g.filter((_, i) => i !== idx)); }}
                      style={{ background: "transparent", border: "none", color: "#C1272D", cursor: "pointer", fontWeight: 700, fontSize: 14 }}
                    >
                      ×
                    </button>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            <button onClick={() => setGruppiScala40((g) => [...g, []])} style={styles.annullaBtn}>+ Nuovo gruppo</button>
          </div>
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <button
              onClick={() => {
                emitAzione("confermaScendiScala40", { gruppiIds: gruppiScala40 });
                setModalitaScala40(false);
                setGruppiScala40([]);
              }}
              style={styles.confermaBtn}
            >
            ...
            <button onClick={() => { setModalitaScala40(false); setGruppiScala40([]); }} style={styles.annullaBtn}>Annulla</button>
          </div>
        </div>
      )}

      {/* Azioni normali (mani 1-6, 8, 9, 10) */}
      {stato.manoAttuale && sonoIoDiTurno && !modalitaScendi && !modalitaScendiLibero && !modalitaChiusura && !modalitaScala40 && stato.manoAttuale.id !== 7 && (
        <div style={styles.azioni}>
          ...
          {!stato.sceso[mioIndice] && (
            <button
              onClick={() => {
                if (stato.manoAttuale.id === 10) {
                  setGruppiScala40([[]]);
                  setModalitaScala40(true);
                } else {
                  setGruppiProposti(stato.manoAttuale.richiesta.map(() => []));
                  setModalitaScendi(true);
                }
                setSelezione([]);
              }}
              disabled={!stato.haPescato}
